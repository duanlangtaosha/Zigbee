/*
  ...
  本程序用51单片机经过AT指令控制模块，完成从SMARTLINK 到 手机与模块的绑定，以及云品台的登录，本地数据发给异地手机，手机异地控制测试板上的继电器动作。
  本程序可以移植到STM8这款只要1元钱的单片机上配合AT模块，完成高性价的产品，比如开关插座。主要演示安信可的云平台的试用！以及整个物联网的流程

  注意：变量初始化成0，否者随机  数组注意溢出问题  0X55 在其他数据中有可能出现，并造成混乱，主要是计数  过滤数据，根据网络环境不同，返回有可能变化，注意通用性
  内部 ID KEY 输出格式已经固定为8个字节模式	  本程序用的AT固件版本是1.01 ，内置合法芯片ID 和授权KEY ，并且这条记录已经在安信可云服务器内存在
  安信可的云有多简单？
  1 登录：    55 26 00 A0 00 60 96 96 60 02 DD 1E 00 00 00 00 00 00 08 00 00 00 00 00 00 00 00 00 00 00 00 00 00 1E DD 26 AC 55
  2 发送数据：55 28 00 AA EE EA 8F 8F EA 02 DD 1E 00 00 00 00 00 00 08 4A 42 85 A9 94 85 0C 00 54 02 45 4D 50 3A 32 35 35 3B 00 42 5C 55
  只要构造出上面的数据包，经过UDP 发送到安信可云服务器，就能通讯了
   
  时间仓促，完成功能后，未经过优化代码优化，和语法风格修改，供参考。辅助文档有这个程序设计框架解释，和详细的指导，这些文件均可以在安信可官网
  WWW.AI-THINKER.COM  ai-cloud 云平台板块得到最新的资讯
...
/////////////////////////////////////////////////////////////////////以上部分是STC的掉电存储程序，读写内部的EEPROM 保存上网账户和密码////////////////////////
...
void make_bander_data()
{
  ...
  //后面的 "light","123456a"  为本设备名字，和本设备的密码，暂时未用 可随便填写
...
void main(void)
{
	...
	{
	        IapEraseSector(IAP_ADDRESS);  //擦除客户EEPROM扇区 512字节
			LED2=0;
			BEEP=1;
			Delay2(200);
		    LED2=1;
	...
	Delay2(300);
	if(IapReadByte(IAP_ADDRESS+EEPROM_LAB)==0x55) //开机后检测是否已经保存了客户的信息在EEPROM中
	have_config=0;
	else
	have_config=1; //EEPROM没有记录客户的路由器账户和密码，需要配置
	///////////////////////////////////////////////////////////////////////////////////以上部分主要完成串口的初始化////////////////////////////
	for(;;)
	{	 
		...
		have_smartlink=1;				 //否则设置已经进入过SMARTLINK 状态
		...
		{
		   if(have_config)//需要客户重新设置路由器账户和密码
		   {
			 for(i=0;recd_buf[i]!=0x0d;i++)
			 {
				ssid_len++;
				ssid[i]=recd_buf[i];			          //拷贝串口缓冲区中的用户路由器名字
				IapProgramByte(IAP_ADDRESS+EEPROM_SSID_LC+i+1,recd_buf[i]);    //存储客户账户到EEPROM中
			 }
			 IapProgramByte(IAP_ADDRESS+EEPROM_SSID_LC,ssid_len);			   //存储客户的账户长度到EEPROM中
			 for(i=0;recd_buf[ssid_len+11+i]!='\r';i++)
			 {
				password[i]=recd_buf[ssid_len+11+i];	                       //拷贝串口缓冲区中的用户路由器密码
				IapProgramByte(IAP_ADDRESS+EEPROM_PASSWORD_LC+i+1,password[i]);//存储客户密码到EEPROM中
				pasd_len++;													   
			 }

			 IapProgramByte(IAP_ADDRESS+EEPROM_PASSWORD_LC,pasd_len);          //存储客户的密码长度到EEPROM中
			 IapProgramByte(IAP_ADDRESS+EEPROM_LAB,0x55);
		   }
		   else	//无需用SMARTLINK 方式获取路由器账户和秘密，账户和密码直接从单片机的EEPROM直接读取
		   {
		    ssid_len=IapReadByte(IAP_ADDRESS+EEPROM_SSID_LC);	 //从EEPROM中的账户长度
			pasd_len=IapReadByte(IAP_ADDRESS+EEPROM_PASSWORD_LC);//从EEPROM中的密码长度
		    for(i=0;i<ssid_len;i++)
			{
			ssid[i]=IapReadByte(IAP_ADDRESS+EEPROM_SSID_LC+i+1); //从EEPROM中读取账户信息到RAM中
			}
			for(i=0;i<pasd_len;i++)
			{
			password[i]=IapReadByte(IAP_ADDRESS+EEPROM_PASSWORD_LC+i+1); //从EEPROM中读取密码信息到RAM中
			}
		   }
		//code unsigned char AT_CWJAP[]="AT+CWJAP=\"360we\",\"zty0012001\"";
			n=0;
			...
			temp_buf[n]='"';			 //以上语句构造出：AT+CWJAP="360we","zty0012001" 这样的指令，其中路由器账户和密码 随客户变动
		}
		...
		if(have_id)					 //从串口中获取了芯片的ID 信息 和KEY 以及FLASH 信息
		{
		have_id=0;
		k=0;
        ...
        //下面的语句将获取的16进制字符换成 16进制数据，8个字节的字符最总得到四个字节的16进制数据 比如字符串"A2345678"最终变成0XA2，0X34，0X56，0X78 四个字节存放
	    for(i=0;i<8;i++)            
	    {
		...
		Delay2(2000);
        at_uart_send_str(temp_buf); //temp_buf 中此时存放的是  AT+CWJAP="360we","zty0012001"	这样的AT指令，这一步加入路由器中
		Delay2(10000);
		at_uart_send_str(AT_CIPMUX);//进入多连接模式，单连接其实也可以的
		Delay2(2000);
        at_uart_send_str(CIPSTART);	//用UDP方式连接到安信可的云，连接到目标5001安信可云服务，顺便监听 2468 端口，code unsigned char CIPSTART[]="AT+CIPSTART=4,\"UDP\",\"cloud.ai-thinker.com\",5001,2468,0"
		Delay2(2000);
		
		if(have_config)			   //客户需要重新配置，这个时候也顺便把模块和手机的绑定关系建立：原理是模块把芯片ID和KEY经过UDP发给手机，手机把这个合法ID上报给服务器，从而绑定！
		{						   //过程是手机在和模块同一个局域网后，手机发送UDP到2468端口，而上一步，模块在监听2468端口！手机发送的UDP包内容是RPL:"192.168.1.10","48008"
		do						   //其实手机是把自己现在的IP地址192.168.1.10 和即将监听的端口48008 告诉模块，然后模块就可以把自己的芯片ID 和KEY授权 各四个字节 回送给手机
		{						   //因此任何一个手机只要获得这个模块的芯片ID 和KEY ,即可向服务器利用这个信息，进行控制权申请，也就是绑定！
		LED1=0;
		LED2=0;
		LED3=0;
		Delay2(200);
		...
		if(have_bander)
		{
		 ...
		 make_AT_CIP3();			   	 //根据上边 在2468 端口监听到手机透露的IP地址和端口 构造一条建立连接指令如右边示意：AT+CIPSTART=3,"UDP","192.168.1.10",48008,2469,0
		 at_uart_send_str(temp_buf);	 //发送出上面构造的连接到AT指令，模块得以执行上面构造的AT指令
		 Delay2(2000);
		 make_bander_data();		     //从绑定手机和设备关系的处理中，构造出如右边的UDP数据：RPT:"0x00FE6738","0xB8B3C281","192.168.0.123","light","123456a"
		 at_uart_send_str(CIP3SEND_LEN); //发送AT指令，告诉模块即将经过路由器发给手机的数据长度，  code unsigned char CIP3SEND_LEN[]="AT+CIPSEND=3,63";
		 Delay2(1000);
		 at_uart_send_str(temp_buf);	 //发送构造好的UDP数据！
		 Delay2(2000);
		 }								 //根据上面的几个步骤，手机获取到了模块的芯片ID 和 KEY ID，手机得到后会利用这个信息让服务器绑定它们两个 
