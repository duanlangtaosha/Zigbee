...
//根据上边 在2468 端口监听到的 广播内容 包括广播者的IP 地址192.168.1.10和端口 48008新建一个3连接，
...
void test()						  //把接收到的数据包转义过来，0X55 转义成0X54 0X01 0X54 替换成0X54 02
{
...
{
	char tt,i,k,n,z=0;
	//str_cut(send_buf,recd_buf,40,5,';');
	test();
	//////////////////////////////////////////////////////////////////////////////////////下面部分为定时器以及串口初始化/////////////////////
	B_TX1_Busy = 0;
	RX1_Cnt = 0;
	...
	{	 
		...
		if(have_smartlink)
		{
			...
			{
				password[i]=recd_buf[ssid_len+11+i];
				pasd_len++;
			}
		//code unsigned char AT_CWJAP[]="AT+CWJAP=\"360we\",\"zty0012001\"";
			n=0;
		    ...
		    temp_buf[i]=0;
			for(i=0;i<10;i++,n++)
			temp_buf[n]=AT_CWJAP[i];
			for(i=0;i<ssid_len;i++,n++)
			temp_buf[n]=ssid[i];
			temp_buf[n]='"';
			n++;
			temp_buf[n]=',';
		...
		for(i=0;recd_buf[i+k]!=';';i++);
		n=i;
		k=(k+n);
		tt=8-i;		  //+CSYSID:CHIP:00FE6738;FLASH:001640EF;KEY:81C2B3B8;	 兼容以前AT指令输出芯片CHIP:FE6738 甚至 CHIP:123 没有固定的长度，后来更改为固定8个字符。

		for(i=0;tt<8;tt++,i++)				  //i k n tt 6 6 6 8
		chip_id[tt]=recd_buf[i+(k-n)];
		k+=7;
		...
		esp_dev_id[7]=((chip_id[6])<<4)+chip_id[7];
		...
		flash_id[7]=((pass_id[6])<<4)+pass_id[7];
...
void fit_stb(unsigned char *str_p)
{
